...
const AnimatedContent = ({ slice }: { slice: Content.HeroSlice }) => {
  ...
  useGSAP(
    () => {
      ...
      const tl = gsap.timeline({ defaults: { ease: "power2.inOut" } }); //using timeline will animate items oneby one if we use gsap directly then it will animate simultaniously
      tl.fromTo(
        ".hero__heading",
        { scale: 0.5 },
